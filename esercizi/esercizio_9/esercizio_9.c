...
int main( int argc, char *argv[] )
{
	...
	if (myrank==0){
		...
		risultatoArray=(int*)malloc(dim*sizeof(int));
    ...
    for (i=0; i<numprocs; i++){
     if (myrank==i){
       ...
       printf("Sono il processo %d. Ho ricevuto l'array e l'ho moltiplicato per a: %d \n",myrank, scalare_a);
     ...
     MPI_Barrier(MPI_COMM_WORLD);
  	...
  	//Gatherv
  	MPI_Gatherv(receiveArray, dimRecv[myrank], MPI_INT, risultatoArray, dimRecv, displs, MPI_INT, 0,MPI_COMM_WORLD);
  	
  	if (myrank==0){
  	
  	   printf("Sono il processo %d. Ho ricevuto l'array risultato\n",myrank);

  	   for(i=0; i<dim; i++){
         printf("risultatoArray[%d] = %d \n",i,risultatoArray[i]);
